...
public class ContourManipulator {
	 ...
	 * @return
	 */
	public Mat centrarImagen(Mat imagenACentrar){
		List<MatOfPoint> contornos = findContours(imagenACentrar);	
		RectContour contornoRectangularMayor = findMaxRectangleContour(contornos, imagenACentrar);
		return findWarpPerspective(imagenACentrar, contornoRectangularMayor);
	}
	...
	private List<MatOfPoint> findContours(Mat img){
		...
		Imgproc.cvtColor(img, imgGrises, Imgproc.COLOR_BGR2GRAY);
		...
		//Pintar contornos
		drawContour(contornos, img, hierarchy);
		return contornos;
	}
	 ...
	 * @return
	 */
	private RectContour findMaxRectangleContour(List<MatOfPoint> contornos, Mat img){
		double maxArea = -1;
	    int IMaxArea = -1;
		...
		Mat m = new Mat(img.size(), CvType.CV_8U);
	 ...
	 * @return
	 */
	private Mat findWarpPerspective(Mat img, RectContour rectangulo){
		...
		if(anguloInclinacion > 0){
			...
			//restarle 1(o alguna pequeña cantidad) a alturaMaxima y a anchoMaximo.
			...
			Imgproc.warpPerspective(img, retorno, perspectiveTransform, new Size(anchoMaximo, alturaMaxima), Imgproc.INTER_CUBIC);
		}
