...
public class WekaClassifier {
	 ...
	 */
	public void saveDataSetIntoFile(Instances setToSave, String fileName){
		
		try {
			PrintWriter writer = new PrintWriter(LOAD_SAVE_PATH + fileName.trim() + ".arff");
			writer.println(setToSave.toString());
			writer.close();
			writeLog("[saveDataSetIntoFile]: Se ha guardado la informaci�n del data set en " +
	 ...
	 * @throws IOException
	 */
	public Instances LoadDataSetFromFile(String fileName){		
		...
		try {
			...
			writeLog("[LoadDataSetFromFile]: Se carg� correctamente el archivo " + fileName.trim() + ".arff");
			return returnValue;
	...
	public Classifier loadClassifierFromFile(String fileName){
		...
		try {
			...
			writeLog("[loadClassifierFromFile]: Se carg� correctamente el clasificador de tipo " + 
	 ...
	 */
	public void testClassifierModel(Classifier classificationModel, String trainingSetFileName,
		...
		try {
			...
			if(trainingSet != null && testSet != null){
				...
				writeLog("[testClassifierModel]: Se finaliz� correctamente la evaluaci�n del modelo " +
	...
	public void saveEvaluationData(Evaluation ev, String fileName){
		...
		try {
			...
			String s = ("**************" + separator + separator +
					...
					//********************************************************
					//	Modificar la matriz de confusi�n para que se ajuste a 
					//	nuestro problema.
					//********************************************************
					
					separator + "Class-1" + "   " + "Class-2" + "Class-E"+ "   -> Se clasific� como" + separator +
			...
			writer.println(s);
			writer.close();
			writeLog("[saveEvaluationData]: Los resultados de la evaluaci�n se han guardado en " +
			"el archivo " + fileName + ".txt");
			
		} catch (FileNotFoundException e) {
			
			writeLog("[saveEvaluationData]: No se pudo guardar el resultado de la evaluaci�n ");
			e.printStackTrace();
		}
	}
